...
bool PhysicsSystem::isCollided(Collider2D* a, Collider2D* b)
{
	...
	vec3 xAxis[64]; // note : a maximum of 32 vertices per poly is supported
	int iNumAxes = 0;
	...
	for (int j = aSize - 1, i = 0; i < aSize; j = i, i++)
	{
		...
		vec3 E1 = aVertices[i];
		vec3 E = E1 - E0;
		xAxis[iNumAxes] = vec3(-E.y, E.x, 0.f);

		if (!IntervalIntersect(aVertices, bVertices, xAxis[iNumAxes], dist))
		{
			return false;
		}
...
bool PhysicsSystem::IntervalIntersect(const std::vector<vec3>& A, const std::vector<vec3>& B,
	const vec3& xAxis, const vec3& xOffset)
{
	float min0, max0;
	float min1, max1;
